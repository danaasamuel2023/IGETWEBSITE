...
const BundleFilter = () => {
  ...
  useEffect(() => {
    const fetchBundles = async () => {
      try {
        ...
        setFilteredBundles(response.data.data.filter(b => b.type === selectedType));
  ...
  useEffect(() => {
    setFilteredBundles(bundles.filter(b => b.type === selectedType));
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      const response = await axios.post('https://iget.onrender.com/api/orders/placeorder',
        { recipientNumber, capacity: selectedBundle.capacity, price, bundleType: selectedBundle.type },
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center">
      <div className="flex flex-col items-center gap-3">
        <div className="animate-spin rounded-full h-8 w-8 border-2 border-gray-200 border-t-yellow-400"></div>
      ...
      <div className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-5">
          <div className="flex items-center gap-3">
            <MtnLogo size={36} />
            <div>
              <h1 className="text-lg font-semibold text-gray-900 dark:text-white">MTN Data Bundles</h1>
              <p className="text-xs text-gray-500 dark:text-gray-400">Choose a bundle and purchase instantly</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-5">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                selectedType === type.id
                  ...
                  : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700 hover:border-yellow-300 dark:hover:border-yellow-600'
              }`}
            >
              {type.label}
            </button>
        ...
        {filteredBundles.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {filteredBundles.map((bundle) => {
              ...
              return (
                ...
                >
                  {/* Yellow top — capacity */}
                  <div className="bg-yellow-400 px-4 py-4 flex flex-col items-center">
                    <MtnLogo size={28} />
                  ...
                  <div className="bg-gray-900 px-4 py-3 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400">{bundle.validity || 'Non-Expiry'}</p>
                    </div>
                    <div className="w-7 h-7 rounded-full bg-yellow-400 flex items-center justify-center group-hover:bg-yellow-300 transition-colors">
          ...
          <div className="bg-gray-900 rounded-xl p-10 text-center flex flex-col items-center">
            <MtnLogo size={48} />
            <p className="text-sm text-gray-400 mt-3">No bundles available for this type.</p>
          </div>
        )}
      </div>
            ...
            <div className="px-5 py-4 bg-gray-50 dark:bg-gray-900/50">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{selectedBundle.validity || 'Non-Expiry'} validity</p>
                </div>
              ...
              {purchaseStatus?.success ? (
                ...
                <div>
                  ...
                  <button
                    ...
                    className="w-full mt-4 py-2.5 rounded-lg text-sm font-medium transition-colors disabled:cursor-not-allowed bg-yellow-400 hover:bg-yellow-500 text-black disabled:bg-gray-200 disabled:text-gray-400 dark:disabled:bg-gray-700 dark:disabled:text-gray-500"
                  >
                    {processingOrder ? (
